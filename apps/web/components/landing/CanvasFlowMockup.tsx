...
const StackedBars = () => (
  <div className="space-y-2.5">
    {STACK_ROWS.map((r, ri) => {
      ...
      return (
        <div key={r.label} className="grid grid-cols-[76px_1fr] items-center gap-2 sm:grid-cols-[120px_1fr] sm:gap-3">
          <div className="truncate text-[10px] text-right sm:text-[11px]" style={{ color: 'var(--hex-ink-soft)' }}>{r.label}</div>
...
export const DashboardMock = () => {
  ...
  return (
    <div ref={ref} className="hex-card overflow-hidden w-full">
      <div className="flex items-center justify-between gap-2 border-b hex-line-soft px-3 py-2.5 sm:px-4" style={{ borderBottomWidth: 1 }}>
        <div className="flex min-w-0 items-center gap-2">
          <span className="truncate text-[12px] font-medium">CanvasFlow · Onboarding Survey</span>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <button className="hidden rounded border hex-line-soft px-2.5 py-1 text-[11px] sm:block" style={{ borderWidth: 1 }}>Edit</button>
          <button className="rounded px-2.5 py-1 text-[11px] text-white" style={{ background: 'var(--hex-ink)' }}>Share</button>
        </div>
      </div>

      <div className="p-3 sm:p-5">
        <h3 className="text-[16px] font-semibold tracking-tight sm:text-[20px]">Onboarding Survey · Q3 Overview</h3>
        ...
        >
          {tabs.map((t) => (
            <div key={t} className={`hex-tab shrink-0 ${tab === t ? 'active' : ''}`} onClick={() => setTab(t)}>{t}</div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mt-4 sm:gap-3">
          {[
            ...
            <div key={k.l} className="hex-kpi px-2 py-2.5 sm:px-3.5 sm:py-3">
              ...
              <div className="mt-1.5 text-[10px] sm:text-[11px]" style={{ color: 'var(--hex-ink-soft)' }}>{k.l}</div>
              ...
              <div className="hex-mono mt-1 hidden text-[9px] sm:block" style={{ color: 'var(--c-teal)' }}>{k.sub}</div>
            </div>
          ))}
        </div>
...
export const FormBuilderMock = () => {
  ...
  return (
    <div className="hex-card flex w-full max-w-140 flex-col overflow-hidden sm:min-h-120">
      <div className="flex items-center justify-between gap-2 border-b hex-line-soft bg-[#fafaf7] px-3 py-2.5 sm:px-4" style={{ borderBottomWidth: 1 }}>
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="hex-mono truncate text-[10px] opacity-60 sm:text-[11px]" style={{ color: 'var(--hex-ink-muted)' }}>canvasflow.app/builder</span>
        </div>
        <div className="flex shrink-0 items-center gap-3">
          <span className="hex-mono text-[10px] opacity-40">Draft</span>
          {/* Redundant next to "Draft" once space is tight. */}
          <span className="hex-mono hidden text-[10px] sm:inline" style={{ color: 'var(--hex-ink-muted)' }}>autosaved · 2s ago</span>
        </div>
      </div>
      ...
      <div className="grid grow grid-cols-1 sm:grid-cols-[160px_1fr]">
        ...
        >
          <div className="hex-mono mb-3 hidden text-[9px] font-bold tracking-wider uppercase opacity-40 sm:block">Content Blocks</div>
          ...
          <div className="hidden pt-6 sm:block">
            <div className="text-[9px] uppercase tracking-wider opacity-40 mb-3 hex-mono font-bold">More</div>
            <div className="px-2.5 py-2 rounded flex items-center gap-2.5 opacity-60 italic">Toggle · Date · Time</div>
          </div>
        </aside>
        <div className="grow space-y-6 p-4 sm:space-y-8 sm:p-8" style={{ background: '#fcfbf7' }}>
          <div className="group/q relative">
            ...
            <div className="text-[15px] font-semibold leading-tight tracking-tight sm:text-[17px]">What&rsquo;s the biggest pain in your current workflow?</div>
          ...
          <div className="group/q relative">
            ...
            <div className="text-[15px] font-semibold leading-tight tracking-tight sm:text-[17px]">How often does this friction occur?</div>
...
export const CanvasEditorMock = () => {
  ...
  return (
    <div className="hex-card overflow-hidden w-full max-w-295 mx-auto min-w-0 group/canvas shadow-[0_32px_64px_-16px_rgba(26,29,41,0.1)] transition-all duration-500 hover:shadow-[0_32px_64px_-16px_rgba(26,29,41,0.2)]">
      <div className="flex items-center justify-between gap-2 border-b hex-line-soft px-3 py-2.5 sm:px-4" style={{ borderBottomWidth: 1 }}>
        <div className="flex min-w-0 items-center gap-2">
          <span className="truncate text-[12px] font-medium">CanvasFlow · Customer Feedback Form</span>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <button className="hidden cursor-pointer rounded border hex-line-soft px-2.5 py-1 text-[11px] transition-colors hover:bg-slate-50 sm:block" style={{ borderWidth: 1 }}>Preview</button>
          <button className="cursor-pointer rounded px-2.5 py-1 text-[11px] text-white transition-opacity hover:opacity-90" style={{ background: 'var(--hex-ink)' }}>Publish</button>
        </div>
      </div>

      <div className="p-3 sm:p-5">
        <h3 className="text-[16px] font-semibold tracking-tight sm:text-[20px]">Form Builder · Interactive Canvas</h3>
        <p className="text-[12px] mt-1" style={{ color: 'var(--hex-ink-soft)' }}>
        ...
        >
          {tabs.map((t) => (
            <div key={t} className={`hex-tab shrink-0 cursor-pointer ${tab === t ? 'active' : ''}`} onClick={() => setTab(t)}>{t}</div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mt-4 sm:gap-3">
          {[
            ...
            <div key={k.l} className="hex-kpi px-2 py-2.5 sm:px-3.5 sm:py-3">
              <div className="text-[15px] font-semibold leading-none sm:text-[20px]">{k.v}</div>
              <div className="mt-1.5 text-[10px] sm:text-[11px]" style={{ color: 'var(--hex-ink-soft)' }}>{k.l}</div>
              <div className="hex-mono mt-1 hidden text-[9px] sm:block" style={{ color: 'var(--c-purple)' }}>{k.sub}</div>
            </div>
          ))}
        </div>
        ...
        <div className="relative mt-5 flex h-80 min-h-0 overflow-hidden rounded-lg border hex-line-soft bg-[#fcfbf7] shadow-sm sm:h-105">
            ...
            <div className="custom-scrollbar flex-1 overflow-x-hidden overflow-y-auto px-4 py-5 sm:px-6 sm:py-6 sm:pt-7">
             <div className="mx-auto w-full max-w-140 space-y-6">
                ...
                >
                   ...
                   <div className="text-[11px] font-bold opacity-30 hex-mono mb-1.5 uppercase tracking-widest">Rating · required</div>
                   <div className="text-[13px] font-semibold mb-3 text-slate-800">How likely are you to recommend CanvasFlow?</div>
                   <div className="flex gap-1.5">
                      {[1, 2, 3, 4, 5].map(n => (
                        <div key={n} className="grow h-8 rounded border hex-line-soft flex items-center justify-center text-[10px] font-medium opacity-60 hover:opacity-100 hover:bg-indigo-50 transition-all cursor-pointer">
                          {n}
                        </div>
                      ))}
                   </div>
                ...
                >
                   ...
                   <div className="flex justify-between items-start mb-1.5">
                      <div className="text-[11px] font-bold text-indigo-600 hex-mono uppercase tracking-widest">Long text · optional</div>
          ...
          <div className="z-10 hidden w-68 shrink-0 transform flex-col border-l hex-line-soft bg-white/95 shadow-[-10px_0_20px_rgba(0,0,0,0.02)] backdrop-blur-sm transition-transform duration-500 min-h-0 min-w-0 md:flex">
             ...
             <div className="p-4 flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-5">
                ...
                <div className="space-y-2.5">
                   <div className="text-[8px] font-bold uppercase tracking-widest opacity-40 hex-mono">Field settings</div>
                   <div className="space-y-1.5">
                     {[
                       { k: 'Type', v: 'Long text' },
                       { k: 'Required', v: 'Off' },
                       ...
                       <div key={row.k} className="p-2.5 rounded bg-indigo-50/40 border border-indigo-100/50 text-[10px] leading-snug flex items-center justify-between gap-2">
